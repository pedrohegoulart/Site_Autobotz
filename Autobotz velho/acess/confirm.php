<?php
    session_start(); 
        //Incluindo a conexão com banco de dados   
    include_once("Db.class.php");    
    //O campo usuário e senha preenchido entra no if para validar
    if((isset($_POST['usuario'])) && (isset($_POST['senha']))){
        $usuario = mysqli_real_escape_string($conn, $_POST['usuario']); //Escapar de caracteres especiais, como aspas, prevenindo SQL injection
        $senha = mysqli_real_escape_string($conn, $_POST['senha']);
        $senha = md5($senha);
            
        //Buscar na tabela usuario o usuário que corresponde com os dados digitado no formulário
        $result_usuario = "SELECT * FROM usuarios WHERE Email = '$usuario' && Senha = '$senha' LIMIT 1";
        $resultado_usuario = mysqli_query($conn, $result_usuario);
        $resultado = mysqli_fetch_assoc($resultado_usuario);

        //Atualizar como Logado
        if(isset($resultado)){
            $logado_usuario = "UPDATE `usuarios` SET `Status`= 1 WHERE Email = '$usuario' && Senha = '$senha' LIMIT 1";
            $logado_update = mysqli_query($conn, $logado_usuario);
        }
        
        //Encontrado um usuario novamente como status atuaizado
        $result_usuario = "SELECT * FROM usuarios WHERE Email = '$usuario' && Senha = '$senha' LIMIT 1";
        $resultado_usuario = mysqli_query($conn, $result_usuario);
        $resultado = mysqli_fetch_assoc($resultado_usuario);
        
        if(isset($resultado)){
            $logado_usuario = "UPDATE `usuarios` SET `Status`= 1 WHERE Email = '$usuario' && Senha = '$senha' LIMIT 1";
            $logado_update = mysqli_query($conn, $logado_usuario);

            $_SESSION['usuarioCPF'] = $resultado['CPF'];
            $_SESSION['usuarioNome'] = $resultado['Nome'];
            $_SESSION['usuarioHierarquia'] = $resultado['Hierarquia'];
            $_SESSION['usuarioEmail'] = $resultado['Email'];
            $_SESSION['usuarioStatus'] = $resultado['Status'];

            if($_SESSION['usuarioHierarquia'] == "1"){
                header("Location: administrativo.php");
            }elseif($_SESSION['usuarioHierarquia'] == "15"){
                header("Location: ../sistema/");
            }else{
                header("Location: ../login/");
            }
        //Não foi encontrado um usuario na tabela usuário com os mesmos dados digitado no formulário
        //redireciona o usuario para a página de login
        }else{    
            //Váriavel global recebendo a mensagem de erro
            $_SESSION['loginErro'] = "Usuário ou senha Inválido";
            header("Location: ../login/");
        }
    //O campo usuário e senha não preenchido entra no else e redireciona o usuário para a página de login
    }else{
        $_SESSION['loginErro'] = "Usuário ou senha inválido";
        header("Location: ../login/");
    }
?>